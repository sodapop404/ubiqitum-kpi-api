...
// ====================================================================
// SYSTEM PROMPT (full Ubiqitum spec)
// ====================================================================
...
// ====================================================================
// Helpers: normalize numeric fields, clamp, round, ±0.01, flag fallback
// ====================================================================
function normalizeValue(value: number | null, seedInt: number) {
  if (value === null) return { value: null, fallback: true };
  ...
  if (s.endsWith("00") || s.endsWith("50")) {
    v = Math.round((v + (seedInt % 2 === 0 ? 0.01 : -0.01) + Number.EPSILON) * 100) / 100;
  }
  return { value: v, fallback: false };
}

// ====================================================================
// Safe JSON parser for messy LLM outputs
// ====================================================================
function safeParseLLMResponse(text: string) {
  try {
    ...
    console.warn("Failed to parse LLM JSON, returning all nulls", err);
    const fallbackObj: Record<string, any> = {};
    REQUIRED_KEYS.forEach((k) => (fallbackObj[k] = null));
    return fallbackObj;
  }
}
...
// ====================================================================
function computeOverall(data: Record<string, any>) {
  const { brand_consideration_percent, brand_trust_percent, brand_relevance_percent, brand_awareness_percent } = data;
  const values = [brand_consideration_percent, brand_trust_percent, brand_relevance_percent, brand_awareness_percent];

  if (values.some((v) => v === null)) return { value: null, fallback: true };

  const overall = 0.35 * brand_consideration_percent +
                  0.30 * brand_trust_percent +
                  0.20 * brand_relevance_percent +
                  0.15 * brand_awareness_percent;
  return normalizeValue(overall, 0); // seed=0 for overall
}
...
export const handler: Handler = async (event) => {
  ...
  // Example fetch to OpenAI-compatible endpoint
  ...
  // ----------------------------------------------------------------
  // Normalize numeric fields + compute overall
  // ----------------------------------------------------------------
  const seed = Number.isInteger(body.seed) ? body.seed : 0;
  const output: Record<string, any> = {};

  for (const k of REQUIRED_KEYS) {
    if (k === "ubiqitum_overallagainastallcompany_score") continue;

    if (typeof parsed[k] === "number") {
      output[k] = normalizeValue(parsed[k], seed);
    } else if (parsed[k] === null) {
      output[k] = { value: null, fallback: true };
    } else {
      // Meta/string field
      output[k] = { value: parsed[k], fallback: false };
    }
  }

  // Compute overall score
  output["ubiqitum_overallagainastallcompany_score"] = computeOverall({
    brand_consideration_percent: output.brand_consideration_percent.value,
    brand_trust_percent: output.brand_trust_percent.value,
    brand_relevance_percent: output.brand_relevance_percent.value,
    brand_awareness_percent: output.brand_awareness_percent.value
  });
